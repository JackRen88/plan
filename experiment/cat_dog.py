...
data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),"..", "..", "data", "cat_dog_split")
...
for epoch in range(MAX_EPOCH):
    ...
    for i, data in enumerate(train_loader):
        ...
        inputs, labels = data
        outputs = net(inputs)
        # print("i:{},labels_size:{}".format(i, len(labels)))
        # backward
        optimizer.zero_grad()
...
for i, data in enumerate(valid_loader):
    ...
    type = "cat" if predicted.numpy()[0] == 0 else "dog"

    img_tensor = inputs[0, ...]  # C H W
    img = transform_invert(img_tensor, valid_transform)
    plt.imshow(img)
    plt.title("LeNet got {}".format(type))
    plt.show()
    plt.pause(0.5)
    plt.close()
